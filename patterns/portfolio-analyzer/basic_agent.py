...
def create_basic_agent(user_id: str, session_id: str) -> Agent:
    ...
    """
    system_prompt = """You are a portfolio health analyst. Evaluate an imported portfolio's holdings. Compute: diversification_score (0-100), sector_allocation breakdown, detected risk_profile (Conservative/Moderate/Aggressive), market_cap_distribution (large/mid/small-cap percentages). Identify sector concentration risks where any single sector exceeds 30% of total value. Generate rebalancing_suggestions as an array of {action: Add|Remove|Increase|Decrease, ticker, target_allocation_pct, rationale}. Prioritize suggestions that address concentration risks and low diversification. If a user preference profile is available, align suggestions with their risk tolerance and investment horizon. Return structured JSON."""
