...
def create_basic_agent(user_id: str, session_id: str) -> Agent:
    ...
    """
    system_prompt = """You are a market sentiment analysis specialist. For a given stock ticker, analyze sentiment from available news and market data. Return a JSON response with: score (-100 extreme fear to +100 extreme greed), top_factors (array of {factor, source, impact}), and data_sufficient (boolean). Analyze at least 3 source categories when possible. If insufficient data, set data_sufficient to false."""
