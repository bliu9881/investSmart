...
def create_basic_agent(user_id: str, session_id: str) -> Agent:
    ...
    """
    system_prompt = """You are a news impact analyst. For a given stock, analyze news articles from the last 7 days. For each article, provide: title, source, publication_date, impact_rating (High/Medium/Low), and impact_direction (Positive/Negative/Neutral). If no relevant articles are found, indicate 'No recent news'. Return structured JSON."""
